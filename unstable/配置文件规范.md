...
* 安装路径：
  ...
  * $DSG\_DATA\_DIR/configs/：用于放置开发库所携带的描述文件，如 $DSG\_DATA\_DIR/configs/org.dtk.core.json，此目录下的配置描述文件为所有程序共享
* 描述文件包含下列属性：
...
* override 文件放置路径：
  * 1：$DSG\_DATA\_DIR/configs/overrides/$appid/foo.example/
  * 2：$DSG\_DATA\_DIR/configs/overrides/org.dtk.core/
...
* global 级别：\$DSG\_APP_DATA/configs/foo.example.json。如果此目录不存在，或者无权限写入，则回退到使用用户级别的目录存储。
...
* contents：保存的配置项的内容，每一项是一个 json 对象，项之间的相对顺序无意义。每一项可包含下列属性：
  * value：保存修改后的值
  * time：值的保存时间，使用 UTC 时间，采用 ISO 8601 表示方法。如果此时间值晚于当前时间，则应当使用当前时间代替它，但不要更新文件中存储的值，当前系统时间变化时不用做任何操作。在使用此配置项的存储值之前，应当先与配置描述文件中定义的时间属性做比较（需遵守 override 机制），只有存储时间比它晚或无法从描述文件获取一个有效的时间时才能使用此值，否则忽略此处存储的值
  * user：记录修改此项设置的用户名称
