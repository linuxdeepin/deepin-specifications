...
* $DSG\_XDG_DATA: 当值为空或不存在时，默认使用 “/usr/share/dsg/”
* $DSG\_DATA_DIR: 当值为空或不存在时，默认使用 “/var/dsg/data”

> 以上 “$FOO” 变量皆为标准的系统环境变量
...
* 安装路径：
  ...
  * $DSG\_XDG\_DATA/configs/：用于放置开发库所携带的描述文件，如 $DSG\_XDG\_DATA/org.dtk.core.json，此目录下的配置描述文件为所有程序共享
* 描述文件包含下列属性：
...
* contents：配置项的内容，每一个配置项是一个 json 对象，配置项之间的相对顺序无意义，且不支持嵌套。每一项配置可包含下列属性：
  ...
  * time：值对应的时间，使用 UTC 时间，采用 ISO 8601 表示方法。使用场景：假设程序 “A” 的配置项 “a” 记录其是否已经进行了初始化，之后 “A” 可能更改了初始化相关的代码，需要确保版本更新之后能重新进行初始化，则可以将配置项 “a” 的 “time” 属性改为一个较新的时间，则旧版 “A” 程序所记录的配置项 “a” 将失效，以此确保更新 “A” 之后能再次进行初始化工作。此配置项可以省略。
...
* override 文件放置路径：
  * 1：$DSG\_XDG\_DATA/configs/overrides/$appid/foo.example/
  * 2：$DSG\_XDG\_DATA/configs/overrides/org.dtk.core/
  * 3：/etc/dsg/configs/overrides/$appid/foo.example/
  * 4：/etc/dsg/configs/overrides/org.dtk.core/
  
* 对于第 2 和第 4 类路径，其省略了 $appid，放置在此目录下的文件对所有应用程序都生效，表示为所有应用程序提供对 org.dtk.core 配置的覆盖
* `$DSG\_XDG\_DATA/configs` 下的路径用于放置安装包携带的文件
...
* global 级别：\$DSG\_DATA_DIR/$appid/configs/foo.example.json
...
* contents：保存的配置项的内容，每一项是一个 json 对象，项之间的相对顺序无意义。每一项可包含下列属性：
  * value：保存修改后的值
  * time：值的保存时间，使用 UTC 时间，采用 ISO 8601 表示方法。如果此时间值晚于当前时间，则应当使用当前时间代替它，但不要更新文件中存储的值。在使用此配置项的存储值之前，应当先与配置描述文件中定义的时间属性做比较（需遵守 override 机制），只有存储时间比它晚或无法从描述文件获取一个有效的时间时才能使用此值，否则忽略此处存储的值
  * user：记录修改此项设置的用户名称
